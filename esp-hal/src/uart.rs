    ...
    T: Instance,
{
    ...
    fn init(){
        ...
        system.perip_clk_en0().modify(|_,w| {
            match T::uart_number() {
                0 => w.uart_clk_en().set_bit(),
                1 => w.uart1_clk_en().set_bit(),
                _ => panic!("Unknown Uart Device!")
            }
        });
        system.perip_rst_en0().modify(|_,w| {
            match T::uart_number() {
        ...
        T::register_block().clk_conf().modify(|_,w| w.rst_core().set_bit());
        system.perip_rst_en0().modify(|_,w| {
            match T::uart_number() {
        ...
        T::register_block().clk_conf().modify(|_, w| w.rst_core().clear_bit());
